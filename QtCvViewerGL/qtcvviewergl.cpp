...
void CQtOpenCVViewerGl::openFile(const QString &imageFile)
{
    this->showImage(cv::imread(imageFile.toStdString()));
}
...
void CQtOpenCVViewerGl::renderImage()
{
    ...
    if (!mRenderQtImg.isNull())
    {
        ...
        glPushMatrix();
        {
            ...
            glRasterPos2i( mPosX, image.height() - mPosY );
            glPixelZoom( 1, -1 );
            // <--- Centering image in draw area
...
bool CQtOpenCVViewerGl::showImage(const cv::Mat& image)
{
    if(image.depth() != CV_8U) {
        //Normalize image
        cv::normalize(image,mOrigImage,0, 255, cv::NORM_MINMAX, CV_MAKETYPE(CV_8U,image.channels()));
    } else {
        //Copy image
...
bool CQtOpenCVViewerGl::showImage(const ip::Image &image)
{
    if(image.type() == ip::Image::IMG_OPENCV_TYPE) {
        return showImage(image.nativeImage<cv::Mat>());
    } else return false;
}
