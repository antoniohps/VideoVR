...
//Occlusion values are > invalidvalue
//
template <class T>
        ...
        for(int i=0;i<src.cols;i++)
        {
            if(s[i]<=invalidvalue)
            {
                ...
                do
                {
                    t++;
                    if(t>src.cols-1)break;
                }while(s[t]<=invalidvalue);//t has the least valid vallue index
...
void stereoTest()
{
    ...
    Mat image = imread( "../PointCloudRendering/test/l/DSCF1674.jpg", CV_LOAD_IMAGE_COLOR);
    Mat imageR = imread( "../PointCloudRendering/test/r/DSCF1674.jpg", CV_LOAD_IMAGE_COLOR);
    Mat destimage;

    /*
    static Ptr<StereoSGBM> cv::StereoSGBM::create 	( 	int  	minDisparity,
            int  	numDisparities,
            int  	blockSize,
            int  	P1 = 0,
            int  	P2 = 0,
    ...
    */
    ...
    StereoSGBM sgbm(1,16*8,3,200,255,1,0,0,0,0,true);
    Mat disp;
    Mat destdisp;
    Mat dispshow;
    ...
    const double focal_length = 1092;//598.57;
    const double baseline = 7.6;//14.0;
...
void kinectTest()
{
    ...
    while (key!='q')
    {
        ...
        projectImagefromXYZ(imni,destim,depthshow,destdisp,xyz,R,t,K,dist,isSub,std::forward<cv::Mat>(mask));
